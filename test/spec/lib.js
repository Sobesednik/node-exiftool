...
const readOnlyFile = path.join(__dirname, '../fixtures/read-only')
const writeFile = path.join(__dirname, '../fixtures/write.data')
...
const libTestSuite = {
    ...
    // },
    createWriteStream: {
        'should reject when permission is denied': () => {
            return lib.createWriteStream()
                .then(() => {
        ...
        'should resolve with a writable': () => {
            return lib.createWriteStream(writeFile)
                .then((ws) => {
